...
public final class CVCalendarMonthContentViewController: CVCalendarContentViewController {
    ...
    public override func showPresentedDate(date: NSDate) {
        guard let presented = monthViews[Presented] else {
            return
        }
        
        let presentedDate = CVDate(date: date)
        let currentDate = CVDate(date: presented.date)
        if matchedMonths(presentedDate, currentDate) {
            return
        }
...
extension CVCalendarMonthContentViewController {
    ...
    public func scrollViewWillBeginDragging(scrollView: UIScrollView) {
        ...
        if translation.x > 0 {
            let monthView = monthViews[Previous]!
            monthView.refreshAccessoryViews()
        } else {
            let monthView = monthViews[Following]!
            monthView.refreshAccessoryViews()
        }
    }
